  ...
  def hrrr_snow(self, spath = None, domain = 'CONUS', type = 'kuchera'):
    ...
    try:
        for i, url in enumerate(urls):
            ...
            barb_int = 16
            snowcf = ax.contourf(lons, lats, snow_type[f'{type}'], cmap=snow_table['snow_cmap'], levels = snow_table['snow_clevs'], norm=mcolors.BoundaryNorm(snow_table['snow_clevs'], snow_table['snow_cmap'].N), transform = ccrs.crs.PlateCarree(), extend = 'max')
            ...
            plt.title(f'{snow_type_title[f'{type}']} Accumulated Snowfall (in)\n{domain} Region', loc = 'left', fontweight = 'bold', fontsize = 10)
            plt.title('Valid: {}\n{} HRRR F{:03d}'.format(ad+timedelta(hours=i),ad.strftime("%Y%m%d %HZ"),i), loc = 'right', fontsize = 10)
            plt.savefig('{}f{:03d}.png'.format(spath, i))
